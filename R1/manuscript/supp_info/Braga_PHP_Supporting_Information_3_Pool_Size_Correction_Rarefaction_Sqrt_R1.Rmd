...
# We will now create a gradient of phylogenetic spatial extents that increments
...
# contains the spatial extent that we have created.

# Finally, to obtain and compare how the phylogenetic relatedness of the
# communities is in relation to a given phylogenetic spatial extent, we apply two
# approaches:
...
### Appendix S3. Assessing and correcting for the spatial extent size bias on indices of community phylogenetic relatedness

### Spatial extent size-dependence in indices for phylogenetic community structure

The magnitude of standardized effect sizes of indices for phylogenetic community structure may increase with the number of species in the phylogeny (or size of the geographical extent) used to compute the null models pertinent to these metrics [@sandel2018e].
...
One possible approach to correct for the geographical extent size bias is to repeatedly randomly subsample (*i.e.* rarefy) the community matrix to smaller geographical extent sizes, and then to calculate the average indices for phylogenetic community structure [here, the net relatedness index (NRI) and the nearest taxon index (NTI); see Methods] across the rarefied subset [see @sandel2018e].
...
We begin by demonstrating the influence of spatial extent sizes on the phylogenetic community structure indices and the robustness of the bias-correction approach with a simple set of simulations.
We generated 10000 communities and a random pure-birth tree, both containing 1000 species, which represents the broad-scale spatial extent.
We then subset 5000 communities and 250 species from the broad-scale spatial extent.

Then, for spatial extent sizes increasing from 250 species to 1000 species, in increments of 50 species, we iteratively calculated the uncorrected net relatedness index (NRI) and the nearest taxon index (NTI) of these simulated communities, as well as two indices corrected through rarefaction -- $\mathsf{NRI_{raref_{FIX}}}$, $\mathsf{NTI_{raref_{FIX}}}$, $\mathsf{NRI_{raref_{REL}}}$ and $\mathsf{NTI_{raref_{REL}}}$ --, which differed in the number of species subsampled in each computation.

$\mathsf{NRI_{raref_{FIX}}}$ and $\mathsf{NTI_{raref_{FIX}}}$ were computed by rarefying the community presence-absence matrix to the lowest species pool possible for that geographical extent, and then computing NRI and NTI on this matrix (as in Sandel, 2018).
$\mathsf{NRI_{raref_{REL}}}$ and $\mathsf{NTI_{raref_{REL}}}$ rarefy the community presence-absence matrix to one third of the number of species in the spatial extent of that region.
...
To assess whether the results from our study still held after correcting for the influence of the size of spatial extents in the standardized effect sizes index values for phylogenetic community structure, we recalculated the NRI and NTI of bat communities using the second bias-correction approach, which ensures less loss information but still corrects for the influence of spatial extent size.
...
### Rarefaction corrects for spatial extent size bias in both simulations and in empirical data

As demonstrated in Sandel [-@sandel2018e], the traditional community phylogenetic relatedness indices (NRI and NTI) increased with the size of the phylogenetic spatial extent in our simulated communities [as shown by $\mathsf{NRI}$ (net relatedness index) and $\mathsf{NTI}$ (nearest taxon index); Figure S3.1 A and B].
However, the rarefaction-based calculation of phylogenetic relatedness effectively removes the spatial extent size bias in the simulations (see Figure S3.1 C-F).
$\mathsf{NRI_{raref_{FIX}}}$ and $\mathsf{NTI_{raref_{FIX}}}$, which fixes the size of spatial extents to the smallest sample,
...
\noindent 
**Figure S3.1.** The size of the phylogenetic spatial extent influences community phylogenetic relatedness index values of simulated communities [as shown by $\mathsf{NRI}$ (net relatedness index) and $\mathsf{NTI}$ (nearest taxon index); A and B].
Rarefaction with both fixed-size sub-sampling [$\mathsf{NRI_{raref_{FIX}}}$ and $\mathsf{NTI_{raref_{FIX}}}$; C and D] and relative-size sub-sampling [$\mathsf{NRI_{raref_{REL}}}$ and $\mathsf{NTI_{raref_{REL}}}$; E and F] removes the spatial extent size-dependence.
...
\noindent 
**Figure S3.2.** Realm-comparison of the spatial extent size bias-corrected phylogenetic structure of bat assemblages -- measured through the (A) rarefied net relatedness index $\mathsf{NRI_{raref}}$) and (B) nearest taxon index $\mathsf{NRI_{raref}}$) -- across a gradient of spatial extent spatial restrictions (see Methods).
spatial extents were restricted for the (i) global, (ii) east-west hemispheric (New World vs. Old World), (iii) biogeographical realm, (iv) tectonic plate, (v) within-realm biome, and (vi) ecoregional extents.
...
\noindent 
**Figure S3.3.** Average bias-corrected rarefied net relatedness index ($\mathsf{NRI_{raref}}$) of worldwide bat communities across the percentiles of historical change in temperature, historical change in precipitation and in situ net diversification rates across geographical extents.
...
```{r NRI-rarefac-quantiles-paleoclim-stab-net-div-i, eval=TRUE, echo = FALSE, fig.align="center", fig.height=7.5*6, fig.width=7.5*3, message=FALSE, warning=FALSE}
...
\noindent 
**Figure S3.4.** Average bias-corrected rarefied nearest taxon index ($\mathsf{NTI_{raref}}$) of worldwide bat communities across the percentiles of historical change in temperature, historical change in precipitation and in situ net diversification rates across geographical extents.
*In situ* net diversification rates were obtained by subtracting the difference between community-weighted means weight-standardized for speciation and extinction rates.
Paleoclimatic legacies for each bat community were obtained as the historical change in climate since the last glacial maximum (LGM); specifically, the mean annual temperature (MAT) or logarithm of the mean annual precipitation (MAP) from the contemporary period minus the estimated mean annual temperature or logarithm of the mean annual precipitation from 22,000 years ago (see Methods).
...
\noindent 
**Figure S3.5.** Partial bootstrapped coefficients for the effects of paleoclimatic legacies and *in situ* diversification rates in the phylogenetic structure of bat communities across spatial geographical extents.
...
```{r plot-coe-i, echo=FALSE, fig.dim=c(5*6, 5), message=FALSE, warning=FALSE, fig.align = 'center', dpi = 250}
...
```{r log-inv-res-table-1-i, echo = FALSE, message = FALSE, warning = FALSE}
library(gtsummary)
library(huxtable)
library(mosaic)
