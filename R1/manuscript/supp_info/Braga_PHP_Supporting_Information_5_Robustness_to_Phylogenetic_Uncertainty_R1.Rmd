...
To account for phylogenetic uncertainty, we recalculated the phylogenetic relatedness and the _in situ_ diversification rates of bat communities on a subset of randomly sampled trees from the posterior distribution of phylogenetic trees from Faurby and Svenning [-@faurby2015mpae] [see @donoghue1996ptrslbbs]. The phylogenetic relatedness of bat communities was calculated across 50 randomly sampled phylogenetic trees, while diversification rates were estimated on 100 randomly sampled phylogenetic trees. We were limited from including higher sample sizes because of restrictions in computational resources. We then computed the average net relatedness index, average nearest taxon index and average community-weighted net diversification rates across the calculations done with the sampled phylogenetic trees and used these averages to reproduce the figures testing each hypothesis from our study.

These results were qualitatively similar to the ones computed on the maximum credibility clade phylogenetic tree (see Figures 2, 3, 4 and 5, and Figures S5.1, S5.2, S5.3 and S5.4), providing evidence that our inferences are robust the uncertainty of phylogenetic hypotheses from the posterior distribution of the phylogenetic relationships we used in our study.
...
fig.Hemispheric.mean.NTI.phylo.sampling.diff.AnnTemp.LGM_cur.quantile <- quantile.XY(
  ...
  lab_y =c(expression(bar("NTI")["rob"["Hemispheric"]]))
)
...
```{r boot-logistic-binary-outcome-09-phylostr-phylo-sampling-paleoclim-stab-net-div-i, echo = FALSE, eval = TRUE, message=FALSE, warning=FALSE, include=FALSE}
...
```{r plot-glm, echo=FALSE, fig.dim=c(5*6, 5), message=FALSE, warning=FALSE, fig.align = 'center', dpi = 250}
...
tbl_stack(
  list(
    data.frame(glm.boot.AllScales.mean.NRI.phylo.sampling.ClimStab.Div.quantiles.09,
               PhyloStructure = "NRI") %>%
      ...
      mutate(estimate, estimate = as.numeric(estimate),
             variable, Variables = plyr::revalue(variable, 
                                                 c("X.Intercept." = "Intercept",
                                                   ...
                                                   "diff.log.AnnPrec.log.LGM_cur" = "Historical change in precipitation",
                                                   "netDiv_CWM_std_tw_rs_1_mean" = "In situ diversification rates"
                                                 )
             ),
    ...
    data.frame(glm.boot.AllScales.mean.NTI.phylo.sampling.ClimStab.Div.quantiles.09,
               PhyloStructure = "NTI") %>%
      reshape2::melt( id.vars = c("SamplingPool", "PhyloStructure"),
                      variable.name = "variable",
                      value.name = "estimate") %>%
