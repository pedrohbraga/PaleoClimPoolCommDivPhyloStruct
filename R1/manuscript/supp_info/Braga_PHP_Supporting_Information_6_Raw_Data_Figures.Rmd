...
```{r fig_1_1_96, fig.align="center", fig.height=10, fig.width=16.5, message=FALSE, warning=FALSE, dev='png', dpi=300, include=FALSE, optipng=''}
############################################################################ 
### Representing the phylogenetic community structure of bats across the ###
### gradient of sampling pool (or species pool) restriction              ###
############################################################################
...
NRI.Realm.boxplot <- ggplot(filter(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div,
                                   is.na(ID_Realm) == FALSE) %>%
                              filter(!between(nri, -1.96, 1.96)),
                            aes(x = SamplingPool, 
                                y = nri)
) +
  geom_jitter(aes(fill = SamplingPool, 
                  colour = SamplingPool), size = 0.01, alpha = 0.05) +
  geom_boxplot(data = filter(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div,
                                   is.na(ID_Realm) == FALSE) %>%
                              filter(!between(nri, -1.96, 1.96)) %>%
                 filter(nri > 1.96), aes(fill = SamplingPool), alpha = 0.7) +
    geom_boxplot(data = filter(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div,
                                   is.na(ID_Realm) == FALSE) %>%
                              filter(!between(nri, -1.96, 1.96)) %>%
                 filter(nri < 1.96), aes(fill = SamplingPool), alpha = 0.7) + 
  ...
  scale_y_continuous(breaks = round(
    #pretty(MPD.LatLong.AllScales.rarefaction$NRI, n = 7)
    c(min(filter(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div,
                 is.na(ID_Realm) == FALSE) %>%
            filter(!between(nri, -1.96, 1.96)) %>%
            pull(nri), na.rm = T),
      0,
      ...
      10,
      15,
      max(filter(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div,
                 is.na(ID_Realm) == FALSE) %>%
            filter(!between(nri, -1.96, 1.96)) %>%
            pull(nri), 
          na.rm = T))),
    limits = c(min(filter(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div,
                 is.na(ID_Realm) == FALSE) %>%
           filter(!between(nri, -1.96, 1.96))%>%
            pull(nri), na.rm = T) - 0.1,
               max(filter(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div,
                 is.na(ID_Realm) == FALSE) %>%
            filter(!between(nri, -1.96, 1.96)) %>%
            pull(nri), na.rm = T) + 0.1)
...
NTI.Realm.boxplot <- ggplot(filter(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div,
  ...
  ggsci::scale_fill_uchicago() +
  ggsci::scale_colour_uchicago(guide = "none") +
  scale_y_continuous(breaks = pretty(filter(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div,
                                   is.na(ID_Realm) == FALSE) %>%
                              filter(!between(nti, -1.96, 1.96)) %>%
                                pull(nti), n = 5),
                     # round(c(min(MPD.MNTD.LatLong.AllScales.raref.rel.worldClimate.diff.CWM.Div$nti, na.rm = T),
...
\noindent
![]("~/chapter-PaleoClimPoolCommDivPhyloStruct/R1/manuscript/supp_info/Braga_PHP_Supporting_Information_6_Raw_Data_Figures_files/figure-latex/fig_1_1_96-1.png")
