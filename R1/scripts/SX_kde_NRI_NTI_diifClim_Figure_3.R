...
MPD.MNTD.CWM.Div.diff.worldClimate.Global.data <- MPD.MNTD.Chiroptera.Comm.AllScales.CWM.Div %>%
  ...
  left_join(MPD.MNTD.LatLong.diff.Global %>%
              select(
                ID,
                diff.AnnTemp.LGM_cur:cur_bio_12
              ),
            by = "ID"
  ) %>%
  mutate(
    diff.AnnTemp.LGM_cur = diff.AnnTemp.LGM_cur / 10,
    diff.log.AnnPrec.LGM_cur = log(diff.AnnPrec.LGM_cur)
  )
...
(fig.kde.Global.NRI.diff.AnnTemp.LGM_cur.Hpi <- ggplot(
  ...
  aes(
    ...
    scale_y_continuous(
      # breaks = round(seq(min(MPD.MNTD.CWM.Div.diff.worldClimate.Global.data$nri, na.rm = TRUE),
      # max(MPD.MNTD.CWM.Div.diff.worldClimate.Global.data$nri, na.rm = TRUE), length.out = 6), 1),
...
Global.NRI.diff.AnnPrec.LGM_cur.Hpi <- ks::Hpi(
  x = MPD.MNTD.CWM.Div.diff.worldClimate.Global.data %>%
    select(
      nri,
...
kde.Global.NRI.diff.AnnPrec.LGM_cur.Hpi <- ks::kde(
  x = MPD.MNTD.CWM.Div.diff.worldClimate.Global.data %>%
    select(
      diff.AnnPrec.LGM_cur,
      nri
...
dimnames(fhat.kde.Global.NRI.diff.AnnPrec.LGM_cur.Hpi[["estimate"]]) <- list(
  fhat.kde.Global.NRI.diff.AnnPrec.LGM_cur.Hpi[["eval.points"]][[1]],
...
molten.fhat.kde.Global.NRI.diff.AnnPrec.LGM_cur.Hpi <- reshape2::melt(fhat.kde.Global.NRI.diff.AnnPrec.LGM_cur.Hpi[["estimate"]],
  ...
  rename(
    diff.AnnPrec.LGM_cur = Var1,
    nri = Var2
  )

# Represent contours with ggplot() and geom_contour

(fig.kde.Global.NRI.diff.AnnPrec.LGM_cur.Hpi <- ggplot(
  molten.fhat.kde.Global.NRI.diff.AnnPrec.LGM_cur.Hpi,
  aes(
    x = diff.AnnPrec.LGM_cur,
    y = nri
  )
) +
    geom_contour_filled(aes(z = estimate),
    ...
    scale_x_continuous(limits = c(-0.01, 0.01) + range(MPD.MNTD.CWM.Div.diff.worldClimate.Global.data$diff.AnnPrec.LGM_cur,
                                                       na.rm = TRUE
    )) +
    theme_classic(base_size = 17) +
    theme(
...
Global.NTI.diff.AnnPrec.LGM_cur.Hpi <- ks::Hpi(
  x = MPD.MNTD.CWM.Div.diff.worldClimate.Global.data %>%
...
# Compute kde
kde.Global.NTI.diff.AnnPrec.LGM_cur.Hpi <- ks::kde(
  x = MPD.MNTD.CWM.Div.diff.worldClimate.Global.data %>%
...
dimnames(fhat.kde.Global.NTI.diff.AnnPrec.LGM_cur.Hpi[["estimate"]]) <- list(
  fhat.kde.Global.NTI.diff.AnnPrec.LGM_cur.Hpi[["eval.points"]][[1]],
...
(fig.kde.Global.NTI.diff.AnnPrec.LGM_cur.Hpi <- ggplot(
  MPD.MNTD.CWM.Div.diff.worldClimate.Global.data,
  aes(
    x = diff.AnnPrec.LGM_cur,
    y = nti
    ...
    # geom_point() +
    geom_contour_filled(
      data = molten.fhat.kde.Global.NTI.diff.AnnPrec.LGM_cur.Hpi,
      aes(z = estimate),
    ...
    scale_x_continuous(limits = c(-0.01, 0.01) + range(MPD.MNTD.CWM.Div.diff.worldClimate.Global.data$diff.AnnPrec.LGM_cur, na.rm = TRUE)) +
    theme_classic(base_size = 17) +
    theme(
      legend.position = "none",
      legend.direction = "horizontal",
