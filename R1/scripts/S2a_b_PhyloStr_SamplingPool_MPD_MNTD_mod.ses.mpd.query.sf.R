...
#                                                                                    #
# The framework used here uses mod.ses.mpd.query.sf() and mod.ses.mntd.query.sf(),   # 
# which are inspired on the picante::ses.mpd() and picante::ses.mntd() and           #
# include PhyloMeasures::mpd.query() and PhyloMeasures::mntd.query() to allow faster #
# and parallel computation using SNOW (snowfall).                                    #
#                                                                                    #
...
SES.MPD.Chiroptera.World <- mod.ses.mpd.query.sf(
  species.data = Chiroptera.FaurSven.comm,
  phylo.tree = Chiroptera.FaurSven.Phylo.ultra, 
  perms = 999, cores = 20
)
...
Chiroptera.NW.Comm <- match.phylo.comm(Chiroptera.FaurSven.Phylo.ultra, NW.Comm)$comm
Chiroptera.NW.Comm <- Chiroptera.NW.Comm[ , colSums(Chiroptera.NW.Comm) >= 1]
Chiroptera.NW.Tree <- match.phylo.comm(Chiroptera.FaurSven.Phylo.ultra, NW.Comm)$phy

Chiroptera.OW.Comm <- match.phylo.comm(Chiroptera.FaurSven.Phylo.ultra, OW.Comm)$comm
Chiroptera.OW.Comm <- Chiroptera.OW.Comm[ , colSums(Chiroptera.NW.Comm) >= 1]
Chiroptera.OW.Tree <- match.phylo.comm(Chiroptera.FaurSven.Phylo.ultra, OW.Comm)$phy
...
for(i in chosen_Biomes){
  ...
  SubRegion.Biome.Comm. <- Chiroptera.FaurSven.comm[rownames(Chiroptera.FaurSven.comm) %in% row.names(SubRegion.Biome.LatLong), , drop = FALSE]
...
for(j in as.factor(ls(SubRegion.Biome.Comm.Phylo))){
  ...
  SES.MPD.Chiroptera.Biome <- mod.ses.mpd.query.sf(
...
for(i in chosen_Ecoregions){
  ...
  ### Subset the data sets
  SubRegion.Ecoregion.CommPhylo <- match.phylo.comm.(Chiroptera.FaurSven.tree, SubRegion.Ecoregion.Comm.)
...
SES.MNTD.Chiroptera.World <- mod.ses.mntd.query.sf(species.data = Chiroptera.FaurSven.comm,
                                                   phylo.tree = Chiroptera.FaurSven.Phylo.ultra, 
                                                   perms = 999, cores = 20)
# Add everything to a data.frame
