...
MPD.MNTD.diff.worldClimate.Global <- diff.worldClimate %>%
  left_join(cbind(filter(MPD.LatLong.Env.AllScales, 
                  ...
                  filter(MNTD.LatLong.Env.AllScales, 
                         SamplingPool == "Global sampling")[, c(30:35, 37, 38)]), 
            by = c("ID" = "ID")) %>%
...
(NRI.diff.AnnTemp.LGM_cur.Global.plot <- ggplot(MPD.MNTD.diff.worldClimate.Global, 
   ...
   scale_y_continuous(breaks = pretty(MPD.MNTD.diff.worldClimate.Global$NRI, n = 7)) +
...
ggsave(filename = "NRI.diff.AnnTemp.LGM_cur.Global.D.plot.png", 
       dpi = 300,
       width = 6.25, height = 6.75, 
       units = "in")

# Divide by biogeographical Global, in the horizontal direction
NRI.diff.AnnTemp.LGM_cur.Global.plot + 
...
(NRI.diff.AnnPrec.LGM_cur.Global.plot <- ggplot(MPD.MNTD.diff.worldClimate.Global, 
...
# NRI.diff.AnnPrec.LGM_cur.Global.plot
...
(NTI.diff.AnnTemp.LGM_cur.Global.plot <- ggplot(MPD.MNTD.diff.worldClimate.Global, 
                                                     aes(x = diff.AnnTemp.LGM_cur/10, 
                                                         y = NTI,
                                                         group = factor(ID_Realm),
                                                         colour = factor(ID_Realm))) +
...
# NTI.diff.AnnTemp.LGM_cur.Global.plot
...
NTI.diff.AnnPrec.LGM_cur.Global.plot
